// Player do som de tempestade de areia — loop com fade in/out, espelha heatPlayer. Best-effort:
// erros (autoplay bloqueado, arquivo ausente) falham em silêncio. Respeita mute + volume mestre.

import { getVolume, isMuted, subscribeMuted, subscribeVolume } from './sounds.ts'

const SAND_SRC = '/sounds/weather/sandstorm.mp3'
const SAND_BASE_VOLUME = 0.5
...
function targetVolume(): number {
  return intended && !isMuted() ? SAND_BASE_VOLUME * getVolume() : 0
}
...
export function startSand(): void {
  const a = ensureEl()
  if (!a) return
  intended = true
  void a.play().catch(() => {})
